...
typedef struct {
	int amountColsScreen;
	int amountRowsScreen;
	int amountColsTotal;   // new; width (which is not discarded)
	int amountRowsTotal;   // new; height (which is not discarded)
	int row;      /* amount of rows (previously) on screen */ //XXX: removejkbbb
	int col;      /* amount of columns (previously) on screen*/ //XXX: remove
	Position screenPosition; //< position of next insert. XXX: in cursor
	Position insertPosition; //< position of next insert. XXX: in cursor
	Position cursorPosition; //< cursor position  XXX: in cursor
	Line *buffer;   //< content 
...
static inline bool isSet(enum term_mode flag) { return (term.mode & flag) != 0; }
...
void
tsetdirt(int dirtBegin, int dirtEnd)
{
	LIMIT(dirtBegin, 0, term.row-1);
	LIMIT(dirtEnd, 0, term.row-1);
	for (int i = dirtBegin; i <= dirtEnd; ++i) { term.dirty[i] = 1; }
}
...
{
	if (newAmountCols < 1 || newAmountRows < 1) {
		fprintf(stderr, "tresize: error resizing to %dx%d\n", newAmountCols, newAmountRows);
		return;
	} else if (newAmountCols == term.amountColsScreen && newAmountRows == term.amountRowsScreen) {
		return;
	}
	// Update the existing rows to the new amount of columns in the buffer and alternate buffer.
	if (newAmountCols != term.amountColsTotal) {
		size_t const rowSizeBytes = newAmountCols * sizeof(Glyph);
		// In case the terminal becomes larger, resize each row. Otherwise keep the 
		// rows as-is; when new commands are printed, the new size is respected.
		if (newAmountCols > term.amountColsTotal) {
			for (int row = 0; row < term.amountRowsTotal; ++row) {
				term.buffer[row] = xrealloc(term.buffer[row], rowSizeBytes);
			} //XXX: maybe I'll need a memseet here
			term.amountColsTotal = newAmountCols;
		}
		// In either case update the alternate screen.
		size_t const amountRowsAlt = MIN(term.amountRowsScreen, newAmountRows);
		for (int row = 0; row < amountRowsAlt; ++row) {
			term.alt[row] = xrealloc(term.alt[row], rowSizeBytes);
		}
	}
	// Update the amount of rows in the buffer and alternate buffer.
	if (newAmountRows != term.amountRowsTotal) {
		size_t const rowSizeBytes = newAmountCols * sizeof(Glyph);
		// In case the current screen size exceeds the buffered size, allocate enough memory.
		if (newAmountRows > term.amountRowsTotal) {
			term.buffer = xrealloc(term.buffer, newAmountRows * sizeof(Line));
			for (; term.amountRowsTotal < newAmountRows; ++term.amountRowsTotal) {
				term.buffer[term.amountRowsTotal] = xmalloc(
						term.buffer[term.amountRowsTotal], rowSizeBytes);
			} //XXX: maybe I'll need a memseet here
		}
		term.alt = xrealloc(term.buffer, newAmountRows * sizeof(Line));
		for (int row = term.amountRowsScreen; row < newAmountRows; ++row) {
			term.alt[row] = xmalloc(term.alt[row], rowSizeBytes);
		}
	}
	// Reset the current screen position if the screen size is reduced; repaint the changed regions
	bool const alternateScreenInUse = isSet(MODE_ALTSCREEN);
	if (newAmountRows < term.amountRowsScreen) {
		if (term.cursorPosition.y >= term.screenPosition.y + newAmountRows) { //< move up
			term.screenPosition.y = term.cursorPosition.y - newAmountRows - 1;
			if (!alternateScreenInUse) { tfulldirt(); }
		}
	} else if (!alternateScreenInUse) {
		tsetdirt(term.amountRowsScreen, newAmountRows - 1);
	}

	// Apply the new screen dimensions
	term.amountRowsScreen = newAmountRows;
	term.amountColsScreen = newAmountCols;
	if (alternateScreenInUse) { tfulldirt(); }

	if (newAmountCols > term.col) {
		int* bp = term.tabs + term.col;

		memset(bp, 0, sizeof(*term.tabs) * (newAmountCols - term.col));
		while (--bp > term.tabs && !*bp) /* nothing */ ;
		for (bp += tabspaces; bp < term.tabs + newAmountCols; bp += tabspaces) { *bp = 1; }
	}
	/* update terminal size */
	term.col = newAmountCols;
	term.row = newAmountRows;
	/* reset scrolling region */
	tsetscroll(0, newAmountRows-1);
	/* make use of the LIMIT in tmoveto */
	tmoveto(term.c.x, term.c.y);
	/* Clearing both screens (it makes dirty all lines) */
	c = term.c;
	for (i = 0; i < 2; i++) {
		if (mincol < newAmountCols && 0 < minrow) {
			tclearregion(mincol, 0, newAmountCols - 1, minrow - 1);
		}
		if (0 < newAmountCols && minrow < newAmountRows) {
			tclearregion(0, minrow, newAmountCols - 1, newAmountRows - 1);
		}
		tswapscreen();
		tcursor(CURSOR_LOAD);
	}
